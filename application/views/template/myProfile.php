...
<?php foreach ($users->result() as $d): ?>
	<?php $Status = $this->session->userdata('Status') == 'Mahasiswa' ? 'Mahasiswa' : $_SESSION['Status'];?>
	<div class="card mb-1 col-4">
		<?php if ($d->Foto !== null) {
		...
		<div class="card-body">
            <img id='foto' class="card-img-top mb-2" src="<?= $image;?>" alt="Klik Untuk Cari Foto Diri" title="Ganti Foto">
			...
			<div class="row">
				<div class="col-md">
					...
					<div class="card-text small mt-2"> 
					 <span class='span-phone caption' data-id='<?= $d->ID;?>'> <i class='fas fa-phone'></i>  <?= $d->NoHP === null ? 'klik to add phone number' : $d->NoHP ;?> </span> 
					 	<input type='number' class='field-phone form-control-sm form-control editor' value='<?= $d->NoHP;?>' data-id='<?= $d->ID;?>' style="display: none;"/>
					</div>
				</div>
