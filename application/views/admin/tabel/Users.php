	...
	<div class='table-responsive'>
		<table class="table small">
			<thead style="background-color: #007BFF;">
				<tr>
					<th scope="col" class='w-auto'><?= $users->row()->Status === 'Dosen' ? "NIP" : "NIM" ?></th>
			...
			<tbody>
				<?php foreach ($users->result() as $m): ?>
					<tr id="tabel<?=$m->ID?>" class="list-item tabel<?=$m->ID?> <?php if (empty($m->Password)) { echo 'table-warning'; }?>">
						...
						<td> <?=$m->NoHP;?> </td>
						<td> 
						<?php echo "<span class='badge badge-primary'>".$m->Status."</span>" ; ?>
					</td>
				...
				<tr class="list-item tabel<?=$m->ID?> w-auto" id="formtabel<?= $m->ID;?>" style="display: none">
					...
					<td><select id="prodi" name="updateUser[]" class="form-control-sm" value='<?=$m->IDProdi?>'>
						<?php foreach($prodi->result() as $data) {
							echo "<option value='".$data->IDProdi."; ".$data->Prodi."'>".$data->Prodi."</option>";
						} ?> </select>
					</td>
					<td>
						<select id="konsentrasi" name="updateUser[]" class="form-control-sm">
							<?php foreach($minat->result() as $data) {
								echo "<option value='".$data->IDMinat."; ".$data->Minat."'>".$data->Minat."</option>";
							} ?> </select>
	...
	<div class='container mt-5'>
		...
		<div class='row align-items-center'>
			<div class='col-md'>
				...
				Data <?=$status?> tidak ditemukan. silahkan tambahkan data melalui form di atas.
			</div>
		</div>
	</div>
<?php }?>
